...
AMQPAscoltatore.prototype.publish = function publish(topic, message, done) {
  ...
  if(deviceId) {
    ...
    var publishProps = { headers: { deviceId: deviceId, mqttTopic: topic }, expiration: oneMinute };

    if(endsWith(topic, "/data")){
      this._client_conn.publish(this.dataQueue, String(message), publishProps);
    }
    else if(endsWith(topic, "/data/power")){
      this._client_conn.publish(this.powerQueue, String(message), publishProps);
      console.log("publishing on " + this.powerQueue + " rabbit queue, topic: " + topic)
    }
    else {
      console.log("not publishing on any rabbit queue, topic: " + topic)
    }
  }
