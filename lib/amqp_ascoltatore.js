...
function AMQPAscoltatore(opts) {
  ...
  this._startConn();
  this.workQueue = "mqtt-broker:device:data";
}
...
AMQPAscoltatore.prototype.publish = function publish(topic, message, done) {
  ...
  if(this._opts.babylon){
    var deviceId = topic.split("/")[2]
    if(deviceId) {
      this._client_conn.publish(this.workQueue, String(message), { headers: { deviceId: deviceId }, expiration: oneMinute });
    }
  }
