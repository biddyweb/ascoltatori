...
function AMQPAscoltatore(opts) {
  ...
  this._startConn();
  this.dataQueue  = "mqtt-broker:device:data";
  this.powerQueue = "mqtt-broker:device:power";
  this.transmitterConnectQueue = "mqtt-broker:transmitter:connect";
  this.deviceConnectQueue = "mqtt-broker:device:connect";
}
...
AMQPAscoltatore.prototype.publish = function publish(topic, message, done) {
  ...
  function amqpForwardMessage(queue, headers) {
    this._client_conn.publish(queue, String(message),
      { headers: headers, expiration: oneMinute });
  }
  ...
  if(version === "v1") {
    ...
    if(endsWith(topic, "/data")){
      amqpForwardMessage(this.dataQueue, { deviceId: deviceId, mqttTopic: topic });
    }
    else if(endsWith(topic, "/data/power")){
      amqpForwardMessage(this.powerQueue, { deviceId: deviceId, mqttTopic: topic });
    }
  } else if(version === "v2") {
    var collection = topicParts[2];
    var itemId = topicParts[3];
    ...
    if(collection === "transmitter" && eventClass === "presence" && topicParts[5] === "connect"){
      ...
      amqpForwardMessage(this.dataQueue, { deviceId: itemId, mqttTopic: topic });
    }
  }
